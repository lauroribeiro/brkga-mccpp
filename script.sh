...
elite_size=(0.20 0.25)
mutatants_size=(0.10 0.20)
crossover_probability=(0.70 0.80)
restart_strategy=(1 2)
restart_generations=(50 100)
# training
...
# make
# read filenames
headers="filename,population_size,elite_size,mutatants_size,crossover_probability,restart_generations,restart_strategy,seed,objective_value,time_elapsed,stop_time,best_value"
# echo -e $headers >> results.csv

for filename in training/*.txt; do
  ...
  if [ "$filename" != "training/20-DSJC1000.1.txt" ]; then
    continue
  fi
  for p in "${population_size[@]}"; do
    for e in "${elite_size[@]}"; do
      for m in "${mutatants_size[@]}"; do
        for c in "${crossover_probability[@]}"; do
          for r in "${restart_strategy[@]}"; do
            for rg in "${restart_generations[@]}"; do
              # Execute the program 10 times
              for ((seed=0; seed<5; seed++)) do

                # p: 200, e: 0.25, m: 0.10, c: 0.80, rg: 100, r: 1, seed: 0, stop_time: 744.487
                if [ "$p" -eq 200 ] && [ "$(awk 'BEGIN{print ('$e' == 0.25)}')" -eq 1 ] && [ "$(awk 'BEGIN{print ('$m' == 0.10)}')" -eq 1 ] && [ "$(awk 'BEGIN{print ('$c' == 0.80)}')" -eq 1 ] && [ "$rg" -eq 100 ] && [ "$r" -eq 1 ] && [ "$seed" -eq 0 ] && [ "$stop_time" == 744.487 ]; then
                  execute=true
                  echo 'vou executar'
                fi
                ...
                trap 'echo "filename: $filename, p: $p, e: $e, m: $m, c: $c, rg: $rg, r: $r, seed: $seed, stop_time: $stop_time" >> trapped_args; exit 0' SIGINT
                result=$(./brkga-mccpp $filename $p $e $m $c $rg $r $seed $stop_time)
                echo "$filename,$p,$e,$m,$c,$rg,$r,$seed,$result,$stop_time,$best_value" >> results.csv
              done
            done
          done
        done
